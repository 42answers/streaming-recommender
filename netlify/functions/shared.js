...
async function discoverTitles(genreId, providerId, mediaType) {
...
async function discoverSimilar(movieTitle, providerIds, mediaType) {
  ...
  for (const s of suggestions) {
    try {
      ...
      if (!providerIds.some((pid) => providerIdList.includes(pid))) continue;
...
async function getClaudeReview(title, year, plot) {
  const Anthropic = require("@anthropic-ai/sdk");
  ...
  try {
    ...
    const message = await client.messages.create({
      ...
      messages: [{
        role: "user",
        content:
          `Write a 2-line mini review of '${title}' (${year}). ` +
          `Line 1: What makes it worth watching (the hook). ` +
          `Line 2: Who it's best for or a minor caveat. ` +
          `Be specific and opinionated, not generic. No spoilers. ` +
          `Plot for context: ${plot}`,
      }],
    });
